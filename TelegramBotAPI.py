...
def handle_messages(message):
    ...
    #Далее пользователь уже строго авторизован, поэтому можно обновлять данные в методе AddUserData, запрашивая текущие данные из бд
    if current_state == 'logged_out':
        if text == "🔐 Войти в ЛМС":
            ...
            bot.send_message(chat_id, "Раздел FAQ:\n ❓Что делать, если произошла неизвестная ошибка в ходе работы бота?\n 💡Очистите историю диалога с ботом, перезапустите бот, используя соответсвующую команду /start")
    ...
    # --- Авторизация: ввод пароля ---
        ...
        bot.send_message(chat_id, "🕓 Пожалуйста, ожидайте\nПроизводится авторизация в системе LMS, это может занять до 1 минуты")
    ...
    # --- Меню после авторизации ---
        ...
        if text == "📅 Посмотреть дедлайны":
            bot.send_message(chat_id, "🕓 Пожалуйста, ожидайте\nПроизводится получение данных из системы LMS, это может занять до 1 минуты")
